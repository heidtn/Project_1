...
class state_estimator:
  ...
  def estimate_state(self, b, f, encoder_L, encoder_R, ways):
    ...
    self.f = open('angle.txt', 'w+')
    self.f.write(str(self.theta))
    progress(str(self.f.read()))
    self.f.close() 
...
class Joy_interface( JoyApp ):
  ...
  def __init__(self,spec,*arg,**kw): 
    ...
    try:
      opts, args = getopt.getopt(sys.argv[1:],"hwsmaf",["wireless","sensor_disable","motor_disable","autonomous_mode","file_angle"])
  ...
  def onStart( self ):
    # Set up the sensor receiver plani
    if(self.file_angle):
      f = open('angle.txt', 'r+')
      self.file_angle_read = float(f.readline())
      progress("file_angle_read: %s" % str(self.file_angle_read))
      f.close()
    ...
    if(self.file_angle):
      self.sensor.estimator.theta = self.file_angle_read
